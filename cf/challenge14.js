...
let names = ['Mr. Brown', ' Ms. Red', 'Dr. Blue', 'Mrs.', '', 'Ms. Black', 'dr. Green', 'Mrs. Orange', 'Purple', 'Mr.  Pink'];

const screenForNames = (arr) => {
  let answer = [];
  for (let i = 0; i < arr.length; i++) {
    let regex = /^(?:Mr|Mrs|Ms|Dr)\.\s\w+/;
    let screened = regex.test(arr[i]);
    if (screened === true) {
      answer.push(arr[i]);
    }
  }
...
console.log('1. regex names--', screenForNames(names));
...
let uppercase = ['apple', 'banana', 'MacGyver'];

const toTitleCase = (arr) => {
  let answer = [];
  for (let i = 0; i < arr.length; i++) {
    let uppercase = arr[i].charAt(0).toUpperCase();
    let fullWord = uppercase + arr[i].slice(1);
    answer.push(fullWord);
  }
  return answer;
};

console.log('2. title case first letter--', toTitleCase(uppercase));
...
console.log('5. bigger than Luke--', biggerThanLuke(starWarsData));
...
const sortBy = (property, arr) => {
  ...
  } else return arr.sort((a, b) => a.price > b.price ? 1 : -1);
};

console.log('4. sort data--', sortBy(property, dataArr));
...
let url = 'http://www.insecure.com';
let url2 = 'https://secure.com';
let url3 = 'https:/missingslash.org'
...
console.log('5. secure url--', isSecure(url));
...
const detectTicTacToeWin = (board) => {
  ...
  function helpCheck(...positions) {
    for (let i = 0; i < positions.length; i++) {
      if (positions[i].join('') === 'XXX' || positions[i].join('') === 'OOO'){
        return true;
      }
    }
    return false;
...
console.log('6. Tic Tac Toe--', detectTicTacToeWin(board));
