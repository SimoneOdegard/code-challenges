...
console.log('sort characters--', sortStarWarsCharacters(starWarsPeople));
...
console.log('removeThree--', removeThree(idx, arr));
...
let joinArr = ['hello', '301', 'students'];

const joinArray = (arr) => {
  return arr.join(' ');
};

console.log('join array--', joinArray(joinArr));
...
console.log('how much pencil--', howMuchPencil(penStr));
...
console.log('words to char list--', wordsToCharList(charList));
...
const listFoods = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let measurement = arr.indexOf(' ') + 1;
    let item = arr.splice(measurement);
    answer.push(item);
  }
...
console.log('list foods--', listFoods(ingredients));
