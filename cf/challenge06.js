...
console.log('sort by children--', sortByChildren(characters));
...
console.log('get course keys--', getCourseKeys(courseInfo));
...
let courseVal1 = 'Code 301';
let courseVal2 = 'jfdsajfoias';

const checkValues = (obj, value) => {
  return Object.values(obj).includes(value);
};

console.log('check values', checkValues(courseInfo, courseVal1));
...
const updateNumbers = (obj) => {
  ...
  for (let i = 0; i < name.length; i++) {
    answer.push(name[i] + ': ' + number[i]);
  }
  return answer;
};

console.log('update numbers--', updateNumbers(numObj));
...
console.log('children?--', hasChildrenValues(characters, character));
/* ------------------------------------------------------------------------------------------------
...
const hasChildrenEntries = (arr, character) => {
  for (let i = 0; i < arr.length; i++) {
    ...
    let charName = entries[0];
    let children = entries[2];
    if (charName[1] === character) {
      return children[1].length === 0 ? false: true;
    }
  }
  return false;
};
  
  console.log('children entries--', hasChildrenEntries(characters, character));
...
console.log('total characters--', totalCharacters(characters));
...
console.log('house size--', houseSize(characters));
