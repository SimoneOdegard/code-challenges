...
console.log('1. get names reversed--', getNames(names));
...
let arrOfArrs = [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]];

const count = (target, input) => {
  let count = 0;
  for (let i = 0; i < input.length; i++) {
    let answer = input[i].filter(num => num === target)
    count += answer.length;
  }
  return count;
};

console.log('2. count integer--', count(5, arrOfArrs));
...
let arr66 = [[1, 2, 3, 4, 5], [6, 7, 2, 4, 5, 7], [9, 2, 3, 6,]]
...
console.log('3. total sum--', totalSum(arr66));
...
let divArr = [ [0,2,5,4, 'bob'], [2,4,10], [] ];

const divisibleByFiveTwoToThePower = (input) => {
  let answer = [];
  for (let i = 0; i < input.length; i ++) {
    let onlyNums = input[i].filter(i => (typeof(i) === 'number'));
    let fives = onlyNums.filter(i => i % 5 === 0);
    for (let j = 0; j < fives.length; j ++) {
      let divByTwo = Math.pow(2, fives[j]);
      answer.push(divByTwo);
    }
  }
  return answer;
};

console.log('4. divisble by 5, 2 to the pow', divisibleByFiveTwoToThePower(divArr));
...
let findMaleAndFemale = (data) => {
  ...
  let answer = temp.join(' and ').toString();
  return answer;
};

console.log('5. find male and female--', findMaleAndFemale(starWarsData));
...
let findShortest = (data) => {
  let height = Math.min(...data.map(i => i.height));
  return height;
};

console.log('6. find shortest--', findShortest(starWarsData));
