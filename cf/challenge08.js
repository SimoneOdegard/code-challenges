...
let str = 'hello world';
let str2 = 'this is legend of zelda';

function returnTen(str){
  let split = str.split('');
  return split.splice(-10);
}

console.log('1. return ten--', returnTen(str2));
...
let typeNumArr = [1, 'bob', 3];

const typeNum = (arr) => {
  return arr.filter(i => (typeof(i) === 'number'));
};

console.log('2. type num--', typeNum(typeNumArr));
...
let containsArr = ['panda', 'ran', 'and'];

const containsAnd = (arr) => {
  return arr.filter(i => i.includes('and'));
};

console.log('3. contains and--', containsAnd(containsArr));
...
let numArr = [1, 2, 3];
let numArr2 = [13, 4, 29, 30];

const oddValues = (arr) => {
  return arr.filter(i => i % 2 !== 0);
};

console.log('4. odd values--', oddValues(numArr2));
...
let vowelStr = ['gregor','hound','xyz'];

const filterStringsWithVowels = (arr) => {
  return arr.filter(yesVowels);
};

function yesVowels(arr) {
  let regex = /[aeiou]/gm;
  return arr.match(regex);
};

console.log('5. filter vowels--', filterStringsWithVowels(vowelStr));
...
let forbiddenValues = [1, 2, 3];
let goodArr = [1, 2, 3, 4];

const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(i => !forbiddenValues.includes(i));
};

console.log('6. not in first arr--', notInFirstArray(forbiddenValues, goodArr));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter((a) => {if (a.baseStat > minBaseStat) {return a}});
};

console.log('7. greater base stats--', getBaseStatGreaterThan(snorlaxData.stats, 50));
...
const getStatName = (arr, minBaseStat) => {
  let answer = [];
  arr.filter((a) => {if (a.baseStat > minBaseStat) {answer.push(a.stat.name)}});
  return answer;
};

console.log('8. get stat name--', getStatName(snorlaxData.stats, 50));
...
const getCharactersWithoutChildren = (arr) => {
  let answer = [];
  arr.filter((a) => {if (a.hasOwnProperty('children')) {answer.push(a)}});
  return answer;
};

console.log('9. no children--', getCharactersWithoutChildren(characters));
...
console.log('10. no words, even or odd--', evenOddNumericValues(wordNumArr));
